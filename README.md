...
### The four paths to "ArnoldC runs Brainfuck"

| Path | Architecture | Sierpinski | Hello World | What it taught us |
|------|-------------|-----------|-------------|------------------|
| **1** | Hardcoded BF in ArnoldC | impossible | 0.065s | 100-var limit, array simulation |
| **2** | Compiled triple chain (Python build) | 6.8s (HW) | 6.5s | MnM stack machine in ArnoldC |
| **3** | Direct BF interpreter (`bf_vm`) | **0.2s** | 0.087s | Stdin input, 8-bit wrapping |
| **4** | True triple chain (pure ArnoldC) | ~2.5h (est.) | **7.2s** | Three interpreters, no Python |
