...
Path 4 was the purest: one fixed ArnoldC program interprets any MnM program, which can itself interpret any BF program. Three generic interpreters, no Python at runtime.

Path 5 added BF input (`,`) but broke that purity: the MnM BF interpreter is now **compiled per BF program** (comparison chains sized to the specific program). The BF input is dynamic, but the BF program itself is baked into `stdin_base.txt`. This is the classic compiled-program model — compile once, run with any input — but it's no longer a fully generic interpreter chain.
...
- **Sierpinski triangle** in 0.2s (`bf_vm`) or ~10 min per row (triple chain)
- **Chess position rendering** from any FEN string in ~4s (`bf_vm` + chessboard.b)
- **rot13** encryption (840 BF instructions, uses `,` for input)
- **Hello World** through 3 nested interpreters (ArnoldC→MnM→BF) in 7.2s
- **38/38 BF test programs** identical output across all execution paths

### The five paths to "ArnoldC runs Brainfuck"

| Path | Architecture | Any BF? | BF input? | Python at runtime? | What it taught us |
|------|-------------|:---:|:---:|:---:|------------------|
| **1** | Hardcoded BF in ArnoldC | No | No | No | 100-var limit, array simulation |
| **2** | Compiled triple chain | No | No | Build only | MnM stack machine in ArnoldC |
| **3** | Direct BF interpreter (`bf_vm`) | **Yes** | **Yes** | No | Stdin input, 8-bit wrapping, chessboard |
| **4** | True triple chain (pure ArnoldC) | **Yes** | No | No | Three generic interpreters, no Python |
| **5** | Triple chain + BF input | No | **Yes** | No | Input through 3 levels, but BF compiled per program |

Path 4 was the purest (any MnM → any BF). Path 5 added BF `,` input but broke the purity (BF program baked at build time). Path 3 remains the only path that runs any BF program with any input.
