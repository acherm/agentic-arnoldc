...
Could we increase Path 6's limits to fit Sierpinski? We tried — including upgrading ASM from 3.3.1 to 9.7. The wall is the **JVM constant pool** (65,535 entries max, a hard spec limit). With ASM 3.3.1, the failure was a silent corruption (`Invalid this class index`); with ASM 9.7, it's a proper `ClassTooLargeException`. Same boundary either way:
...
| Sierpinski-min | 125 | 133 | 264 | 462K | **CP overflow** | 10 tape cells short |

**10 tape cells short.** The JVM class file format uses `u2` (16-bit) for `constant_pool_count`, capping it at 65,535 entries. Each ArnoldC static field needs ~3 constant pool entries (Utf8 name + NameAndType + FieldRef). No ASM version can fix this — it would require splitting across multiple class files (not supported by ArnoldC).
...
The ArnoldC compiler (written in Scala, using ASM for bytecode generation — upgraded from 3.3.1 to 9.7 during this project) has the value 100 hardcoded in three places:
...
14. **Pure triple chain (Path 6):** Built `build_mnm_bf_generic.py` to generate a FIXED MnM BF interpreter (always the same, 4,654 instructions) that reads any BF program + input from the MnM input queue at runtime. Combined with `mnm_vm` (290K lines), this achieves three fixed generic interpreters: ArnoldC interprets MnM interprets BF, any program, any input, no Python. 38/38 test programs pass including Hello World (111 instructions). Sierpinski doesn't fit (needs 132 tape cells, limit 122)
15. **ASM upgrade (3.3.1 → 9.7):** Attempted to fix what appeared to be an ASM 3.3.1 bug (`Invalid this class index`). Upgraded to ASM 9.7: added `isInterface` parameter to 6 `visitMethodInsn` calls, added `asm-commons` dependency. Result: proper `ClassTooLargeException` error instead of silent corruption, but **same capacity limit**. The wall is the JVM spec (`constant_pool_count` is `u2` = max 65,535), not the library. Sierpinski remains 10 tape cells short
